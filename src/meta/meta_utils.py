...
def collect_episodes(env, policy, device, n_episodes: int, max_steps: int = 200) -> List[Trajectory]:
    trajectories = []
...
def compute_policy_loss(trajectories: List[Trajectory], policy, gamma: float, device) -> torch.Tensor:
    losses = []
    ...
    for obs_arr, act_arr, rew_arr in trajectories:
        ...
        if ret_t.numel() > 1:
            std = ret_t.std()
            if std > 1e-6: # Normalize if std is non-zero
                ret_t = (ret_t - ret_t.mean()) / (std + 1e-8)
            else:
                # If std is 0 (all rewards are identical), just subtract mean
                ret_t = ret_t - ret_t.mean()
        else:
             # If single element, cannot normalize (effectively becomes 0)
             ret_t = ret_t - ret_t.mean()
